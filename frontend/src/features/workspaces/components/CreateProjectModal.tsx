...
const CreateProjectModal: React.FC = () => {
  const { isProjectModalOpen, closeProjectModal } = useWorkspaceUIStore();
    ...
    <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center z-[110] p-4">
      <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
        <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
          ...
          <button onClick={closeProjectModal} className="text-slate-400 hover:text-rose-500 transition-colors">
            <X size={20} />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-6">
          ...
          <div className="mt-8 flex items-center justify-end gap-3 pt-4 border-t border-slate-100">
            <button type="button" onClick={closeProjectModal} className="px-5 py-2 rounded-lg text-sm font-bold text-slate-500 hover:bg-slate-100">
              Cancel
            </button>
            <button type="submit" disabled={isPending || !name.trim()} className="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-2 rounded-lg text-sm font-bold flex items-center gap-2 transition-all disabled:opacity-50">
              {isPending ? <Loader2 size={16} className="animate-spin" /> : 'Create Workspace'}
            </button>
          </div>
        </form>
      </div>
