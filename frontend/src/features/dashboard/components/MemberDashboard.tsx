...
const MemberDashboard = ({ data }: { data: any }) => {
  ...
  const {
    ...
    my_focus = [],
    performance_history = [] // Đã đổi tên chuẩn theo API mới
  } = data;
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 font-sans animate-in fade-in duration-500 pb-10">
      ...
      {/* ================= HÀNG 2/CỘT PHẢI: MONTHLY PERFORMANCE ================= */}
      <div className="lg:col-span-2 bg-white p-6 rounded-2xl shadow-sm border border-slate-200 flex flex-col min-h-[450px]">
        <div className="flex justify-between items-start mb-6 shrink-0">
          <div>
            {/* Đổi Title chuẩn Kanban */}
            <h3 className="font-bold text-[15px] text-slate-800">Monthly Performance & Transparency</h3>
            <p className="text-[12px] font-medium text-slate-500 mt-1">Your performance over recent months</p>
          </div>
          <button className="text-slate-400 hover:text-slate-600"><MoreVertical size={18} /></button>
        </div>
        ...
        <div className="flex-1 w-full relative -ml-4">
          <ResponsiveContainer width="100%" height="100%">
            <ComposedChart data={performance_history}>
              ...
              <Bar dataKey="velocity" name="Completed Points" barSize={40} fill="#3b82f6" radius={[4, 4, 0, 0]} />
