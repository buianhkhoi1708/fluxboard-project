...
const CreateTaskModal: React.FC<CreateTaskModalProps> = ({ isOpen, onClose, columnId, columnName }) => {
  ...
  const modalContent = (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 md:p-12">
      ...
      <div className="relative w-full max-w-[900px] bg-slate-50/95 backdrop-blur-xl rounded-[2rem] shadow-2xl flex flex-col max-h-[90vh] md:max-h-[95vh] overflow-hidden animate-in zoom-in-95 slide-in-from-bottom-4 duration-300 border border-white/50">
        
        {/* Header Responsive */}
        <div className="bg-white/80 backdrop-blur-sm px-4 py-4 md:px-6 md:py-5 border-b border-slate-200/60 flex justify-between items-start gap-3 md:gap-6 sticky top-0 z-10">
          <div className="flex-1 min-w-0">
            ...
            />
            <p className="text-[12px] md:text-[13px] text-slate-500 px-2 md:px-3 mt-1 font-medium truncate">
          ...
          <button type="button" onClick={onClose} className="p-2 md:p-2.5 bg-slate-100 text-slate-500 hover:bg-rose-100 hover:text-rose-600 rounded-full transition-all shrink-0 mt-1">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-4 md:p-6 lg:p-8">
          <div className="flex flex-col md:flex-row gap-6 md:gap-8 lg:gap-10">
            ...
            <div className="flex-1 flex flex-col gap-6 md:gap-8">
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full min-h-[120px] md:min-h-[140px] p-4 md:p-5 bg-white border border-slate-200/80 rounded-2xl text-sm md:text-[15px] leading-relaxed text-slate-700 outline-none focus:border-indigo-400 focus:ring-4 focus:ring-indigo-100/50 transition-all resize-y shadow-sm custom-scrollbar"
                />
              </div>
                ...
                <div className="bg-white border border-slate-200/80 rounded-2xl p-2.5 shadow-sm">
                  <div className="flex flex-col gap-1 max-h-48 overflow-y-auto custom-scrollbar pr-1">
                    {subtasks.map((stTitle, idx) => (
                      <div key={idx} className="group/st flex items-center gap-2.5 md:gap-3 p-2 md:p-2.5 hover:bg-slate-50/80 rounded-xl transition-all border border-transparent hover:border-slate-100">
                        <Square size={16} className="md:w-[18px] md:h-[18px] text-slate-300 shrink-0" />
                        <span className="text-sm md:text-[15px] flex-1 truncate text-slate-700 font-medium">{stTitle}</span>
                        <button type="button" onClick={() => handleRemoveLocalSubtask(idx)} className="opacity-100 md:opacity-0 group-hover/st:opacity-100 p-1.5 text-slate-300 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-all">
            ...
            {/* CỘT PHẢI (Thuộc tính) */}
            <div className="w-full md:w-[280px] flex flex-col gap-4 md:gap-6 shrink-0">
              <div className="bg-white p-4 md:p-5 rounded-2xl border border-slate-200/80 shadow-sm flex flex-col gap-4 md:gap-5">
                  ...
                  <div className="flex flex-wrap gap-2 relative">
                      ...
                      assignees.map((userId, idx) => {
                        ...
                        return (
                          <div key={`chosen-${userId || idx}`} className="flex items-center gap-2 px-2 py-1.5 bg-indigo-50 border border-indigo-100 rounded-xl shadow-sm group/name">
                            ...
                            <button type="button" onClick={() => toggleAssignee(userId)} className="opacity-100 md:opacity-0 group-hover/name:opacity-100 p-0.5 hover:bg-indigo-200 rounded-full text-indigo-400 hover:text-indigo-600 transition-all"><X size={10} /></button>
                          </div>
                        );
                      })
                    ) : (
                      <span className="w-full text-xs text-slate-400 italic bg-slate-50 p-2.5 rounded-xl border border-slate-200 border-dashed text-center">Chưa phân công</span>
                    )}
                    ...
                    {isAssigneePopupOpen && (
                      <>
                        ...
                        <div className="absolute top-full mt-2 left-0 md:-right-4 w-64 md:w-72 bg-white border border-slate-200 shadow-xl rounded-xl z-50 p-2 max-h-56 overflow-y-auto custom-scrollbar">
                            ...
                            projectMembers.map((member: any, idx: number) => {
                              ...
                              return (
                                <div key={`pop-${safeId || idx}`} onClick={(e) => { e.preventDefault(); e.stopPropagation(); toggleAssignee(safeId); }} className={`flex items-center justify-between p-2 rounded-lg cursor-pointer transition-colors ${isSelected ? 'bg-indigo-50' : 'hover:bg-slate-50'}`}>
                                  <div className="flex items-center gap-2">
                                    {member.avatar_url ? <img src={member.avatar_url} className="w-7 h-7 rounded-full object-cover" /> : <div className="w-7 h-7 rounded-full bg-slate-200 flex items-center justify-center text-[10px] font-bold text-slate-600">{initial}</div>}
                ...
                <div className="grid grid-cols-2 md:grid-cols-1 gap-4 mt-2 md:mt-0">
                  <div className="flex flex-col gap-2">
              ...
              <div className="bg-white p-4 md:p-5 rounded-2xl border border-slate-200/80 shadow-sm flex flex-col gap-4">
                <div className="flex flex-col gap-2">
                  ...
                  <DatePicker selected={startDate} onChange={(date: Date | null) => setStartDate(date)} selectsStart startDate={startDate} endDate={dueDate} locale="vi" dateFormat="dd/MM/yyyy" placeholderText="Chọn ngày đầu" customInput={<CustomDateInput />} isClearable />
                </div>
                <div className="flex flex-col gap-2">
                  ...
                  <DatePicker selected={dueDate} onChange={(date: Date | null) => setDueDate(date)} selectsEnd startDate={startDate} endDate={dueDate} minDate={startDate || undefined} locale="vi" dateFormat="dd/MM/yyyy" placeholderText="Chọn ngày cuối" customInput={<CustomDateInput />} isClearable />
                </div>
              </div>
            </div>
        ...
        {/* Footer Responsive */}
        <div className="bg-white/90 backdrop-blur-md px-4 py-3 md:px-6 md:py-4 border-t border-slate-200/60 flex flex-col sm:flex-row justify-end gap-3 items-center z-10 shrink-0">
          <button type="button" onClick={onClose} className="w-full sm:w-auto px-5 py-2.5 text-slate-600 hover:text-slate-800 hover:bg-slate-100 bg-slate-50 border border-slate-200/80 rounded-xl text-sm font-bold transition-all">Hủy</button>
          <button type="button" onClick={handleCreate} disabled={isSaving || !title.trim()} className="w-full sm:w-auto flex justify-center items-center gap-2 px-8 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-sm font-bold shadow-md md:shadow-lg shadow-indigo-200 transition-all active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed">
