...
const Column: React.FC<ExtendedColumnProps> = memo(({ list, onOpenTaskDetail }) => {
  ...
  const safeListId = String(list.id || list._id);
  const totalPoints = getColumnTotalPoints(list);
  const tasks = list.tasks || [];

  const { setNodeRef } = useDroppable({
    id: safeListId,
    data: { type: 'List', listId: safeListId, columnId: safeListId }
  });

  useEffect(() => {
    if (isEditingName && editNameInputRef.current) editNameInputRef.current.focus();
  }, [isEditingName]);

  useEffect(() => setEditColName(list.list_name), [list.list_name]);
  ...
  const handleDeleteColumn = async () => {
    ...
    if (tasks.length > 0) {
      alert('Cột này đang có công việc. Vui lòng chuyển công việc sang cột khác trước khi xóa!');
      return;
    }
  ...
  return (
    <div className="w-[85vw] max-w-[300px] sm:w-[300px] shrink-0 flex flex-col bg-slate-100/80 backdrop-blur-md rounded-2xl max-h-full relative border border-white/60 shadow-sm">
      ...
      <div ref={setNodeRef} className="flex-1 overflow-y-auto flex flex-col gap-2.5 px-2 pb-2 custom-scrollbar min-h-[50px]">
        <SortableContext items={tasks.map((task) => String(task.id || task._id))} strategy={verticalListSortingStrategy}>
          {tasks.map((task) => (
            <TaskItem
