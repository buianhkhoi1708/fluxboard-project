...
// 👉 Cải tiến CustomDateInput: Thêm icon ChevronDown để rõ ràng đây là Dropdown
...
const TaskDetailModal = ({ isOpen, onClose, task, listId }) => {
  const { updateTask, toggleSubtask, addSubtask, deleteTask, getMemberById } =
    useBoardStore(); // 🚀 Thêm getMemberById vào đây
  ...
  const modalContent = (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 md:p-12">
      {/* Backdrop mờ ảo, tối hơn một chút */}
      <div
        className="absolute inset-0 bg-slate-900/40 backdrop-blur-md animate-in fade-in duration-300"
        onClick={onClose}
      ></div>

      {/* Box chính: Bo góc to hơn, bóng đổ cực sâu */}
      <div className="relative w-full max-w-[1100px] bg-slate-50/95 backdrop-blur-xl rounded-[2rem] shadow-[0_20px_60px_-15px_rgba(0,0,0,0.3)] flex flex-col max-h-[95vh] overflow-hidden animate-in zoom-in-95 slide-in-from-bottom-4 duration-300 border border-white/50">
        ...
        <div className="bg-white/80 backdrop-blur-sm px-6 py-5 border-b border-slate-200/60 flex justify-between items-start gap-6 sticky top-0 z-10">
          <div className="flex-1">
            ...
            <p className="text-[13px] text-slate-500 px-3 mt-1.5 font-medium flex items-center gap-1.5">
              Trong danh sách{" "}
              <span className="font-bold text-slate-700 bg-slate-100 px-2 py-0.5 rounded-md border border-slate-200/60">
                {task.status}
              </span>
            </p>
          </div>
            ...
            <div className="flex-1 flex flex-col gap-8">
                  ...
                  <div className="flex flex-col gap-1 max-h-64 overflow-y-auto custom-scrollbar pr-1">
                    {(task.subtasks || []).map((st) => (
                      ...
                      >
                        ...
                        <span
                          className={`text-[15px] flex-1 truncate ${st.status === "DONE" ? "line-through text-slate-400" : "text-slate-700 font-medium"}`}
                        >
                          {st.title}
                        </span>
                        <button
                          onClick={() => deleteTask(listId, st.id || st._id)}
              ...
              {task.ai_estimation_reason && (
                <div className="bg-gradient-to-br from-amber-50 to-orange-50/50 border border-amber-200/60 rounded-2xl p-5 flex gap-4 shadow-sm relative overflow-hidden">
                  ...
                  <Sparkles
                    size={120}
                    className="absolute -bottom-6 -right-6 text-amber-500/5 rotate-12"
                  />

                  <div className="p-2.5 bg-amber-100/80 text-amber-600 rounded-xl h-fit shadow-sm backdrop-blur-sm relative z-10">
                    <Sparkles size={22} />
                  </div>
              ...
              <div className="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-sm flex flex-col gap-5">
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {(() => {
                      // 🚀 TUYỆT CHIÊU VÉT CẠN: Bắt mọi kiểu đặt tên mảng của Backend
                      const assigneeArray =
                      ...
                      if (assigneeArray.length > 0) {
                        return assigneeArray.map((userId, idx) => {
                          // Lấy Object User từ Store
                        ...
                        // Trường hợp mảng rỗng hoặc không có field nào khớp
                        return (
        ...
        <div className="bg-white/80 backdrop-blur-sm px-6 py-4 border-t border-slate-200/60 flex flex-col-reverse sm:flex-row justify-between items-center gap-4 z-10">
          ...
          <div className="flex gap-3 w-full sm:w-auto">
            ...
            <button
              onClick={handleSave}
              className="flex-1 sm:flex-none flex justify-center items-center gap-2 px-8 py-3 sm:py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-[15px] font-bold shadow-lg shadow-indigo-200 transition-all active:scale-95 border border-indigo-500"
            >
