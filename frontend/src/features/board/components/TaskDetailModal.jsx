...
import { X, AlignLeft, CheckSquare, Clock, Calendar, Flag, Target, Sparkles, Plus, Square, Save, Trash2, User, ChevronDown } from 'lucide-react';
...
const TaskDetailModal = ({ isOpen, onClose, task, listId }) => {
  ...
  const modalContent = (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 md:p-12">
      ...
      <div className="relative w-full max-w-[900px] bg-slate-50/95 backdrop-blur-xl rounded-[2rem] shadow-[0_20px_60px_-15px_rgba(0,0,0,0.3)] flex flex-col max-h-[95vh] overflow-hidden animate-in zoom-in-95 slide-in-from-bottom-4 duration-300 border border-white/50">
                  ...
                  <div className="flex flex-col gap-1 max-h-64 overflow-y-auto custom-scrollbar pr-1">
                    {(task.subtasks || []).map(st => (
                      <div key={st.id || st._id} className="group/st flex items-center gap-3 p-2.5 hover:bg-slate-50/80 rounded-xl transition-all border border-transparent hover:border-slate-100">
                        ...
                        <button onClick={() => deleteTask(listId, st.id || st._id)} className="opacity-0 group-hover/st:opacity-100 p-1.5 text-slate-300 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-all">
                          <X size={16} />
                        </button>
                      </div>
                    ))}
