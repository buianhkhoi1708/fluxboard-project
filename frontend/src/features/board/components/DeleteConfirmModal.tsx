  ...
  const modalContent = (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
      ...
      <div className="relative w-full max-w-md bg-white rounded-2xl shadow-2xl border border-slate-100 p-5 sm:p-6 animate-in zoom-in-95 duration-200">
        ...
        <button 
          onClick={onClose}
          className="absolute top-3 right-3 sm:top-4 sm:right-4 p-2 sm:p-1.5 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-full transition-all outline-none"
        >
        ...
        <div className="flex flex-col items-center text-center gap-3 sm:gap-4 mt-2 sm:mt-0">
          ...
          <div className="w-14 h-14 sm:w-14 sm:h-14 bg-rose-50 rounded-full flex items-center justify-center text-rose-500 shadow-sm shrink-0">
            <AlertTriangle className="w-7 h-7 sm:w-7 sm:h-7" />
          </div>

          <div className="flex flex-col gap-1.5">
            <h3 className="text-lg sm:text-xl font-bold text-slate-900">Xác nhận xoá Task?</h3>
            <p className="text-sm text-slate-500 px-1 sm:px-2 line-clamp-4">
              Bạn có chắc chắn muốn xoá task <span className="font-bold text-slate-800">"{taskTitle}"</span> không? Hành động này không thể hoàn tác!
            </p>
          </div>

          <div className="flex flex-col-reverse sm:flex-row gap-3 w-full mt-3 sm:mt-2">
            <button 
              onClick={onClose}
