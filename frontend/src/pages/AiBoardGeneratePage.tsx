    ...
    {[1, 2, 3].map(step => (
      <div
        ...
        className={`h-1.5 rounded-full transition-all duration-500 ${
          currentStep >= step ? 'w-6 bg-indigo-500 shadow-sm' : 'w-2 bg-slate-200'
        }`}
      />
    ))}
  </div>
...
const AiBoardGeneratorPage = () => {
  ...
  const { projectId: passedProjectId } = location.state || {};
  ...
  // Lấy mảng projects thực sự (phòng trường hợp pagination)
  ...
  // Tìm role VIEWER để lọc ra khỏi assignees
  ...
  const [selectedMembers, setSelectedMembers] = useState<{ userId: string; roleId: string }[]>([]);
  ...
  // Khi danh sách thành viên thay đổi, tự động chọn tất cả (có thể bỏ nếu không muốn)
  useEffect(() => {
  ...
  }, [fetchProjects, fetchRbacData, projectList.length, systemRoles.length]);

  // --- 5. HANDLERS (useCallback để tránh re-render con) ---
  const toggleMember = useCallback(
    (userId: string, existingRoleId: string) => {
  ...
  const nextStep = useCallback(() => {
    if (currentStep === 1 && !selectedProjectId) {
      return setErrors({ project: 'Sếp chọn 1 Workspace để tiếp tục nhé!' });
    }
    if (currentStep === 2 && selectedMembers.length === 0) {
      return setErrors({ members: 'Cần tích chọn ít nhất 1 nhân sự để đưa vào Board!' });
    }
    setErrors({});
    setCurrentStep(prev => prev + 1);
  }, [currentStep, selectedProjectId, selectedMembers.length]);
  ...
  const handleFinalGenerate = useCallback(async () => {
    ...
    prompt, selectedMembers, viewerRoleId, selectedProjectId,
    generationMode, startDate, generateAiBoard, navigate
  ]);
        ...
        <div className="bg-white/80 backdrop-blur-sm rounded-2xl border border-slate-200/80 shadow-lg overflow-hidden">
              ...
              <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div>
                  ...
                  <p className="text-sm text-slate-500 font-medium mt-1">
                    Chọn không gian làm việc để AI đồng bộ hóa dữ liệu.
                  </p>
                </div>
                        ...
                        return (
                          ...
                          >
                            <div
                              ...
                              onClick={() => toggleMember(safeUserId, roleId)}
                            >
                              <div
                            ...
                            <div className="flex items-center gap-2 shrink-0">
                              ...
                              >
                                {isSelected && (
                                  <CheckCircle2 size={12} className="text-white shrink-0" strokeWidth={4} />
                                )}
                              </div>
                            </div>
                          </div>
            ...
            {currentStep === 3 && (
              <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-500">
                <div>
                  ...
                  <p className="text-sm text-slate-500 font-medium mt-1">
                    Mô tả quy trình để AI tự động phân rã task và tính toán Story Points.
                  </p>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  ...
                  >
                    <div className={`p-2.5 rounded-xl ${generationMode === 'SIMPLE' ? 'bg-indigo-600 text-white' : 'bg-slate-100'}`}>
                      <Layout size={20} />
                    </div>
                  ...
                  >
                    <div className={`p-2.5 rounded-xl ${generationMode === 'ADVANCED' ? 'bg-indigo-600 text-white' : 'bg-slate-100'}`}>
                      <Layers size={20} />
                    </div>
                    <div className="text-left">
                  ...
                  <div className="p-4 bg-white rounded-2xl border-2 border-slate-100 flex items-center gap-4 shrink-0">
                    ...
                    <div className="text-left flex-1">
                      ...
                      <input
                        ...
                        className="text-xs font-bold text-slate-800 outline-none bg-transparent w-full mt-0.5"
                      />
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm relative">
                  <div className="flex gap-2 flex-wrap mb-4">
                    <button
                      onClick={() =>
                        setPrompt(
                          'Xây dựng dự án AI Security: Triển khai ModelGuard bảo vệ mô hình khỏi Model Extraction attacks. Các giai đoạn: Đánh giá lỗ hổng, Thiết kế Defensive Framework, Coding Backend (Java/Quarkus) và Pentest.'
                        )
                      }
                      className="text-[9px] font-black px-3 py-1.5 bg-slate-100 text-slate-600 rounded-full hover:bg-slate-900 hover:text-white transition-all"
                    >
                      + BẢO MẬT AI
                    </button>
                    <button
                      onClick={() =>
                        setPrompt(
                          'Chiến dịch Content Marketing Tết 2026 cho Fluxboard. Gồm các bước: Idea Concept, Thuê KOLs, Video Production và Chạy Ad đa kênh.'
                        )
                      }
                      className="text-[9px] font-black px-3 py-1.5 bg-slate-100 text-slate-600 rounded-full hover:bg-slate-900 hover:text-white transition-all"
                    >
                  ...
                  <textarea
                    ...
                    placeholder="Mô tả chi tiết dự án tại đây..."
                    className="w-full text-sm font-medium outline-none resize-none placeholder:text-slate-300 custom-scrollbar leading-relaxed pr-8"
                  />
                  {prompt && (
                    <button
                      onClick={() => setPrompt('')}
          ...
          <div className="border-t border-slate-200 p-4 md:px-8 flex items-center justify-between bg-white/60 backdrop-blur-sm">
            {currentStep > 1 ? (
              <button
                onClick={() =>
                  passedProjectId && currentStep === 2
                    ? navigate(-1)
                    : setCurrentStep(prev => prev - 1)
                }
                className="flex items-center gap-2 text-xs font-black text-slate-400 hover:text-slate-800 transition-all uppercase tracking-widest"
              >
            ...
            {currentStep < 3 ? (
              <button
                onClick={nextStep}
                className={`flex items-center gap-2 px-8 md:px-10 py-3.5 rounded-2xl text-xs font-black uppercase tracking-widest transition-all shadow-lg active:scale-95 ${
              ...
              <button
                ...
                className={`flex items-center gap-2 px-10 md:px-12 py-3.5 rounded-2xl text-xs font-black uppercase tracking-widest transition-all shadow-xl active:scale-95 ${
                  isProcessing
