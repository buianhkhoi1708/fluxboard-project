...
const OrganizationPage = () => {
  ...
  return (
    <div className="flex-1 bg-slate-50 h-full overflow-y-auto p-6 md:p-10">
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
          <div>
            <h1 className="text-3xl font-black text-slate-800 flex items-center gap-3">
              <div className="p-2 bg-indigo-100 rounded-xl"><Building2 className="text-indigo-600" size={28} /></div>
              Organization Structure
            </h1>
            <p className="text-slate-500 mt-2 font-medium">Quản lý cơ cấu phòng ban, nhóm và nhân sự trong hệ thống.</p>
          </div>
          <button onClick={openCreateDeptModal} className="flex items-center gap-2 bg-gradient-to-r from-indigo-600 to-indigo-700 hover:from-indigo-700 hover:to-indigo-800 text-white px-5 py-2.5 rounded-xl text-sm font-bold shadow-lg shadow-indigo-200/50 transition-all active:scale-95">
            <Plus size={18} strokeWidth={2.5} /><span>Thêm Phòng Ban</span>
          </button>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-20 bg-white rounded-2xl border border-dashed border-slate-300 shadow-sm">
            ...
            <button onClick={openCreateDeptModal} className="text-indigo-600 font-bold bg-indigo-50 px-6 py-2.5 rounded-xl hover:bg-indigo-100 transition-colors">+ Tạo phòng ban ngay</button>
          </div>
        ) : (
              ...
              <div key={dept.id} className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden transition-all hover:shadow-md">
                <div className="bg-gradient-to-r from-slate-50 to-white p-5 border-b border-slate-200 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 bg-white border border-slate-200 shadow-sm rounded-xl flex items-center justify-center shrink-0"><ShieldAlert className="text-indigo-500" size={24} /></div>
                    <div>
                      <h2 className="text-lg font-bold text-slate-800 tracking-tight flex items-center gap-2">
                        {dept.name}
                        {dept.code && <span className="bg-indigo-100 text-indigo-700 text-[11px] px-2 py-0.5 rounded-md font-bold uppercase tracking-wider">{dept.code}</span>}
                      </h2>
                      <p className="text-sm text-slate-500 mt-0.5">Manager: <span className="font-bold text-slate-700">{dept.manager_name || dept.managerName || 'Chưa cập nhật'}</span></p>
                    </div>
                  </div>
                  <button onClick={() => openCreateTeamModal(dept.id)} className="flex items-center gap-2 text-sm font-bold text-indigo-600 bg-indigo-50 border border-indigo-100 px-4 py-2 rounded-xl hover:bg-indigo-100 transition-colors">
                  ...
                  {dept.teams?.length > 0 ? dept.teams.map((team) => (
                    <div key={team.id} className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm hover:border-indigo-200 transition-colors">
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            <h3 className="font-bold text-[15px] text-slate-800 flex items-center gap-2">
                              {team.name}
                              {team.code && <span className="bg-slate-100 text-slate-500 text-[10px] px-1.5 py-0.5 rounded font-bold uppercase">{team.code}</span>}
                            </h3>
                            <p className="text-xs text-slate-500 mt-1">Lead: <span className="font-semibold text-slate-700">{team.lead_name || team.leadName || 'Chưa cập nhật'}</span></p>
                          </div>
                        </div>
                        <button className="text-slate-400 hover:text-indigo-600 p-1 bg-slate-50 rounded-md transition-colors"><MoreVertical size={16} /></button>
                      </div>
                      ...
                      <div className="space-y-2 mb-5">
                        {team.members?.length > 0 ? team.members.map((member) => (
                          <div key={member.id} className="flex justify-between items-center bg-slate-50 p-2.5 rounded-lg border border-slate-100 group hover:bg-white hover:border-indigo-100 transition-all">
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                <p className="text-[13px] font-bold text-slate-700 leading-tight group-hover:text-indigo-600 transition-colors">{member.full_name || member.fullName}</p>
                        ...
                        )) : <div className="text-xs text-slate-400 italic text-center py-4 bg-slate-50/50 rounded-lg border border-dashed border-slate-200">Nhóm này chưa có thành viên.</div>}
                      </div>

                      <button onClick={() => openAddMemberModal(dept.id, team.id)} className="w-full py-2.5 border border-dashed border-slate-300 rounded-xl text-sm font-bold text-slate-500 hover:text-indigo-600 hover:border-indigo-400 hover:bg-indigo-50 transition-all flex justify-center items-center gap-2">
                    ...
                    <div className="col-span-full flex flex-col items-center justify-center py-10 bg-white border border-dashed border-slate-200 rounded-xl">
                      ...
                      <button onClick={() => openCreateTeamModal(dept.id)} className="mt-3 text-sm font-bold text-indigo-600 hover:underline">Tạo Team đầu tiên</button>
                    </div>
                  )}
                </div>
              </div>
      ...
      <OrgFormModal isOpen={modalState.isOpen} mode={modalState.mode} targetDeptId={modalState.targetDeptId} onClose={() => setModalState({ ...modalState, isOpen: false })} />
      <UserPickerModal isOpen={isUserPickerOpen} targetDeptId={targetIds.deptId} targetTeamId={targetIds.teamId} onClose={() => setIsUserPickerOpen(false)} />
    </div>
  );
};
