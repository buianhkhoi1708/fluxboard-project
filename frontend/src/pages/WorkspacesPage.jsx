...
import CreateBoardModal from '../features/workspaces/components/CreateBoardModal'; // 👉 Nhớ check lại path này
import { 
  ...
  return (
    <div className="flex-1 bg-gradient-to-br from-slate-50 via-white to-indigo-50/30 h-full overflow-y-auto no-scrollbar p-4 md:p-6 lg:p-8">
      <div className="max-w-7xl mx-auto">
        ...
        {/* HEADER - Tối ưu với hiệu ứng glassmorphism và khoảng cách tinh tế */}
        ...
        {isLoading ? (
          ...
          <div className="space-y-8">
            {filteredProjects.map((item) => {
              ...
              return (
                <section key={workspace.id} className="bg-white/80 backdrop-blur-sm rounded-2xl border border-slate-200/80 shadow-lg shadow-slate-200/20 p-5 md:p-6 transition-all hover:shadow-xl hover:border-indigo-200/50">
                  {/* Workspace Header - Tinh tế hơn */}
                  <div className="flex items-center justify-between mb-5 pb-4 border-b border-slate-100">
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center shadow-md shadow-indigo-200/50">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-4 text-xs font-semibold text-slate-500 mt-1">
                          <span className="flex items-center gap-1.5 bg-slate-100/80 px-2.5 py-1 rounded-full">
                            <Users size={12} /> {workspace.membersCount || 0} members
                          </span>
                          <span className="flex items-center gap-1.5 bg-slate-100/80 px-2.5 py-1 rounded-full">
                  ...
                  {/* BOARDS GRID - Card đẹp hơn với hover effects */}
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    {/* Create Board Button - Tối ưu hơn */}
                    <button 
                      onClick={() => {
