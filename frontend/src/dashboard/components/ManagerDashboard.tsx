...
const ManagerDashboard = ({ data }: { data: any }) => {
  // 1. Rút data từ prop truyền vào, có fallback an toàn
  ...
  // Tính trung bình tỷ lệ hoàn thành để hiển thị ở giữa Donut Chart
  const avgCompletion = useMemo(() => {
    if (teamCompletion.length === 0) return 0;
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 font-sans animate-in fade-in duration-500 pb-10">
        ...
        <div className="flex-1 w-full relative -ml-4">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={weeklyProgress}>
              ...
              <Tooltip cursor={{stroke: '#cbd5e1', strokeDasharray: '3 3'}} contentStyle={{borderRadius: '8px', border: '1px solid #e2e8f0', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)'}} />
              
              <Line type="monotone" dataKey="fluxboard" name="Fluxboard" stroke="#0284c7" strokeWidth={2.5} dot={false} activeDot={{r: 5}} />
              <Line type="monotone" dataKey="potpan" name="Potpan" stroke="#f59e0b" strokeWidth={2.5} dot={false} activeDot={{r: 5}} />
            </LineChart>
          </ResponsiveContainer>
        </div>
      </div>

      {/* ================= CỘT 2: DONUT CHART (Task Completion Rate) ================= */}
      <div className="bg-white p-5 rounded-xl shadow-sm border border-slate-200 flex flex-col h-[420px]">
        <div className="flex justify-between items-start mb-2 shrink-0">
          ...
          <ResponsiveContainer width="100%" height={200}>
            <PieChart>
              <Pie
                ...
                stroke="none"
              >
                {teamCompletion.map((entry: any, index: number) => (
                  <Cell key={`cell-${index}`} fill={TEAM_COLORS[index % TEAM_COLORS.length]} />
                ))}
              </Pie>
              <Tooltip formatter={(value: number) => [`${value}%`, 'Hoàn thành']} contentStyle={{borderRadius: '8px', border: '1px solid #e2e8f0'}} />
            </PieChart>
          </ResponsiveContainer>
          {/* Text nằm giữa Donut */}
        ...
        {/* 🚀 BẢN FIX: Hiển thị TOÀN BỘ dữ liệu Team, tự động cuộn nếu danh sách quá dài */}
      ...
      {/* ================= CỘT 3: GROUPED BAR CHART (AI Prediction vs Actual) ================= */}
      <div className="bg-white p-5 rounded-xl shadow-sm border border-slate-200 flex flex-col h-[420px]">
        ...
        <div className="flex-1 w-full relative -ml-4">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={aiVsActual}>
              ...
              {/* Cột 1: AI (Xanh dương đậm) */}
              <Bar dataKey="ai_point" name="AI Dự đoán" fill="#1e3a8a" barSize={25} radius={[2, 2, 0, 0]} />
              {/* Cột 2: Thực tế (Xanh ngọc) */}
              <Bar dataKey="actual_point" name="Thực tế" fill="#10b981" barSize={25} radius={[2, 2, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
